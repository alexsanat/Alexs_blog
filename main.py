...
def register():
    ...
    if form.validate_on_submit():
        if User.query.filter_by(email=form.email.data).first():
            flash("You've already signed up with that email, please use log in")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash('Please check the spelling - your Email is incorrect')
            return redirect(url_for('login'))
        elif not check_password_hash(user.password, password):
            flash('Please check the spelling - your Password is incorrect')
            return redirect(url_for('login'))
        else:
            login_user(user)
            return redirect(url_for('get_all_posts'))
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            ...
            flash('You need to log in or registered to leave a comment')
